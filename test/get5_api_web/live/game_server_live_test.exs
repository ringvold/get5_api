defmodule Get5ApiWeb.GameServerLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all game_servers", %{conn: conn, game_server: game_server} do
      {:ok, _index_live, html} = live(conn, ~p"/game_servers")
    ...
    test "saves new game_server", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/game_servers")

      assert index_live |> element("a", "New Game server") |> render_click() =~
               "New Game server"

      assert_patch(index_live, ~p"/game_servers/new")
      ...
      {:ok, _, html} =
        ...
        |> render_submit()
        |> follow_redirect(conn, ~p"/game_servers")
    ...
    test "updates game_server in listing", %{conn: conn, game_server: game_server} do
      ...
      assert index_live |> element("#game_servers-#{game_server.id} a", "Edit") |> render_click() =~
               "Edit Game server"

      assert_patch(index_live, ~p"/game_servers/#{game_server}/edit")
    ...
    test "deletes game_server in listing", %{conn: conn, game_server: game_server} do
      ...
      assert index_live
             |> element("#game_servers-#{game_server.id} a", "Delete")
             |> render_click()
  ...
  describe "Show" do
    ...
    test "displays game_server", %{conn: conn, game_server: game_server} do
      {:ok, _show_live, html} = live(conn, ~p"/game_servers/#{game_server}")
    ...
    test "updates game_server within modal", %{conn: conn, game_server: game_server} do
      {:ok, show_live, _html} = live(conn, ~p"/game_servers/#{game_server}")

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Game server"

      assert_patch(show_live, ~p"/game_servers/#{game_server}/show/edit")
      ...
      {:ok, _, html} =
        ...
        |> form("#game_server-form", game_server: @update_attrs)
        |> render_submit()
        |> follow_redirect(conn, ~p"/game_servers/#{game_server}")
