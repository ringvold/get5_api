...
defmodule Get5Api.Steam.Profile do
  import SweetXml
  ...
  """
  @spec fetch(pos_integer | binary) :: Get5Api.Steam.Profile.t()
  def fetch(community_id_or_custom_url) do
    fetch(community_id_or_custom_url, Req)
  end

  @doc false
  def fetch(community_id_or_custom_url, req) do
    base_url = Get5Api.Steam.SteamID.base_url(community_id_or_custom_url) <> "?xml=1"

    response = req.get(base_url, [])

    case response do
      {:ok, resp} -> resp.body |> from_xml()
  ...
  defp from_xml(doc) do
    error = extract_string(doc, ~x"//response/error/text()")
    ...
    most_played_games =
      for most_played_game <- xpath(profile, ~x"//mostPlayedGames/mostPlayedGame"l) do
        %Get5Api.Steam.Profile.MostPlayedGame{
